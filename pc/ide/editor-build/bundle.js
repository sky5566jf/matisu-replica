var MatisuEditor = (() => {
  ...
  // editor-src.js
  var editor_src_exports = {};
  __export(editor_src_exports, {
  ...
  var MountedTree = class {
    constructor(tree, overlay, parser, bracketed2 = false) {
      this.tree = tree;
  ...
  var lua = {
    ...
    indent: function(state, textAfter, cx) {
      ...
      return state.basecol + cx.unit * (state.indentDepth - (closing2 ? 1 : 0));
    },
    languageData: {
  ...
  var LspClient = class {
    ...
    connect(rootUri) {
      ...
      return new Promise((resolve, reject) => {
        ...
        ws.onclose = () => {
          this.ready = false;
          this._flushErr(new Error("LSP \u8FDE\u63A5\u65AD\u5F00"));
        };
        ws.onerror = () => {
          if (!settled) {
            settled = true;
      ...
      return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
          if (this.pending.has(id)) {
            ...
            reject(new Error("LSP \u8D85\u65F6: " + method));
          }
        }, 15e3);
      });
    }
  ...
  var darkHl = HighlightStyle.define([
    ...
    { tag: tags.standard(tags.variableName), color: "#89ddff" },
  ...
  function posToOffset(doc2, pos) {
    const line = doc2.line(Math.min(pos.line + 1, doc2.lines));
    return Math.min(line.from + pos.character, line.to);
  }
  function offsetToPos(doc2, off) {
    const line = doc2.lineAt(off);
      ...
      try {
        ...
        return {
          ...
          options: items.slice(0, 200).map((it) => ({
            ...
            detail: it.detail ? String(it.detail).slice(0, 80) : void 0,
            info: it.documentation ? typeof it.documentation === "string" ? it.documentation : it.documentation.value || "" : void 0,
  ...
  var LUA_OPEN = /* @__PURE__ */ new Set(["then", "do", "repeat", "function"]);
  var LUA_CLOSE = /* @__PURE__ */ new Set(["end", "until"]);
  var LUA_MID = /* @__PURE__ */ new Set(["else", "elseif"]);
  function scanLuaBlocks(state) {
    ...
    while (i2 < n) {
      ...
      if (c === "-" && text[i2 + 1] === "-") {
        ...
        if (text[i2] === "[") {
          ...
          if (text[j] === "[") {
            ...
            const startLine = doc2.lineAt(startOff).number;
            const endLine = doc2.lineAt(endOff - 1).number;
      ...
      if (/[A-Za-z_]/.test(c)) {
        ...
        const word = text.slice(i2, j);
        i2 = j;
        const prev = (text[i2 - word.length - 1] || "").trim();
        ...
        if (!isField) {
          if (LUA_OPEN.has(word)) {
            if (word === "then" && pendingElseif) {
              ...
              stack.push({ line: doc2.lineAt(i2 - word.length).number, depth: stack.length });
            }
          } else if (LUA_CLOSE.has(word)) {
            const top2 = stack.pop();
            if (top2) {
              blocks.push({ startLine: top2.line, endLine: doc2.lineAt(i2 - word.length).number, depth: top2.depth });
            }
            pendingElseif = false;
  ...
  var _blockCache = { state: null, byLine: null, blocks: null };
  function luaBlocks(state) {
    ...
    for (const b of blocks) {
      ...
      const cur2 = byLine.get(b.startLine);
      if (!cur2 || b.endLine > cur2) byLine.set(b.startLine, b.endLine);
    }
    _blockCache = { state, byLine, blocks };
    return _blockCache;
  }
  var luaFoldService = foldService.of((state, lineStart) => {
  ...
  function createEditor({ parent, doc: doc2, getUri, lspUrl, onChange }) {
    ...
    return {
      ...
      getFoldRanges() {
        return luaBlocks(view.state).blocks.slice();
      },
  ...
  return __toCommonJS(editor_src_exports);
})();
